...
Public swimmer event analytics SHALL expose the current achieved official grade,
the next official grade if one exists, and the full ordered list of applicable
official benchmark thresholds for the swimmer gender and selected event.
...
- **THEN** the system returns no official grade result, no official benchmark
  thresholds, and marks the status as unavailable for that event
